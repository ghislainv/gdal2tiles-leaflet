...
# gdal_translate -co "RESAMPLING=NEAREST" -of MBTiles data/fcc_RGB_masoala_mercator.tif data/fcc_masoala_bioscene.mbtiles
...
#gdal_translate -co "RESAMPLING=NEAREST" -of MBTiles data/fcc_RGB_masoala_christelle_mercator.tif data/fcc_masoala_christelle.mbtiles
#gdaladdo -r nearest data/fcc_masoala_christelle.mbtiles 2 4 8 16

#===== Probability geoserver
gdal_retile.py -v -useDirForEachRow -r near -levels 4 -ps 2048 2048 -co "TILED=YES" -co "COMPRESS=LZW" -co "PREDICTOR=2" -co "BLOCKXSIZE=256" -co "BLOCKYSIZE=256" -targetDir pyramid_prob_MDG data-far-tropics/prob.tif

#=== GDAl COG (Cloud Optimized GeoTIFF)
gdaladdo -r nearest data-far-tropics/prob.tif 2 4 8 16 32 64 128 256 512 1024 2048
gdal_translate -co "COMPRESS=LZW" -co "PREDICTOR=2" -co "TILED=YES" -co "COPY_SRC_OVERVIEWS=YES" -co "BLOCKXSIZE=256" -co "BLOCKYSIZE=256" data-far-tropics/prob.tif data-far-tropics/prob_tiled.tif